...
def calculate_valid_loss():
    ...
    valid_loss /= len(validloader)  # 计算平均损失
    return valid_loss
...
def valid_model(epoch):
    ...
    if psnr > best_psnr:
        ...
        if ('epoch'+str(epoch)) in dellist:
            dellist.remove('epoch'+str(epoch))

        for dl in dellist:
            os.system('rm -r vis'+experiment+'/'+dl)
    
    valid_loss = calculate_valid_loss()  # You need to implement this method
    scheduler.step(valid_loss)
    current_lr = scheduler.get_last_lr()
    print(f'Current learning rate: {current_lr}')

    return psnr, valid_loss  # 返回PSNR和验证损失
...
if __name__ == '__main__':
    ...
    for epoch in range(1, cfg.epochs + 1):
        ...
        if VIS_RESULTS:
            visualize(str(epoch))
            valid_psnr, valid_loss = valid_model(epoch)  # 接收返回的PSNR和验证损失
